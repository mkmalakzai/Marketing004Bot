...
if (get("ban_" + uid, "no") === "yes" && !admin) { Bot.sendInlineKeyboard(row("🎫 Support", "support"), "🚫 Your account is restricted."); return; }
...
if (action === "balance") { var b=[row("➕ Add Funds","deposit_methods"),row("🧾 Transactions","ledger")];show("💰 YOUR BALANCE",money(wallet(uid))+" USD",b);return; }
...
if (action === "admin") {show("🛠 ADMIN PANEL","Owner: "+owner,[row("📁 Categories","admin_cats"),row("🚀 Services","admin_services"),row("📦 Packages","admin_packages"),row("🛒 Orders","admin_orders"),row("💳 Deposits","admin_deposits"),row("💰 Payment Methods","admin_methods"),row("🎁 Offers","admin_offers"),row("🎫 Tickets","admin_tickets"),row("⚙ Settings","admin_settings")]);return;}
...
if (action === "admin_settings") {show("⚙ SETTINGS","Owner: "+owner+"\nAdditional admins: "+(get("admins","")||"—")+"\nProvider: "+(get("api_url","")?"Configured":"Not configured")+"\n\nAPI dispatch is disabled until provider integration is verified.",[row("👥 Set Admin IDs","admin_setting admins"),row("🔗 Provider URL","admin_setting api_url"),row("🔑 Provider Key","admin_setting api_key")]);return;}
if (action === "admin_setting") {if(["admins","api_url","api_key"].indexOf(id)<0)return;ask("admin_setting",id,"Send new "+id+" value. Never share API keys in a public repository.");return;}
